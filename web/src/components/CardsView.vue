<!--
Führerschein und Personalausweis UI-Komponente für FiveM NUI. Zeigt die Daten in einem stilisierten Kartenformat an, mit automatischem Ausblenden nach 5 Sekunden. Unterstützt sowohl moderne NUI-Events als auch Legacy-Methoden für Direct Execute / RAGE MP Legacy Calls.
-->
    ...
    <div v-if="cardType === 'idcard'" class="card id-card">
      ...
      <div class="footer" v-if="idData.firstname && idData.lastname">
        <div class="signature-field">
          ...
          <div class="label" style="text-align: center;">Unterschrift des Inhabers</div>
        </div>
      </div>
    </div>
...
// Formatierung der Account-IDs (SA-00000123)
...
// 1. NUI Event Listener (Moderne Webview-Events)
...
// 2. Globale Legacy-Methoden für Direct Execute / RAGE MP Legacy Calls
const setLegacyIdData = (firstname, lastname, birthdate, gender, accountId, creationDate) => {
  idData.value = {
    firstname: firstname || '',
    lastname: lastname || '',
    birthdate: birthdate || '',
...
const setLegacyLicenseData = (data) => {
  const parsedData = typeof data === 'string' ? JSON.parse(data) : data;
  licenseData.value = {
    ownerName: parsedData.ownerName || '',
    ownerId: parsedData.ownerId || null,
    carData: parsedData.carData || null,
    truckData: parsedData.truckData || null,
    boatData: parsedData.boatData || null,
    aircraftData: parsedData.aircraftData || null
  };
  cardType.value = 'license';
  triggerAutoClose();
};

// Exponieren der globalen Fenster-Funktionen für ExecuteJS
onMounted(() => {
  window.addEventListener('message', handleNuiMessage);
  
  // Kompatibilität mit altem idcard.js execute (`setData('Max', 'Mustermann', ...)`)
  window.setData = setLegacyIdData;

  // Kompatibilität mit altem licenseui.html execute (`setData(jsonData)`)
  window.setLicenseData = setLegacyLicenseData;
});

onUnmounted(() => {
  window.removeEventListener('message', handleNuiMessage);
  delete window.setData;
  delete window.setLicenseData;
});
</script>
...
.license-row .icon { width: 30px; text-align: center; color: #00ffff; font-size: 16px; }
...
.license-row .status { font-size: 20px; font-weight: bold; width: 20px; text-align: center; }
